---
title: 'Project 2: Integer Programming'
...
Prior to any analysis, the data was read into R using the provided script 'readData.R'.

# 1. Daily Stock Returns

```{r}
# Initialize matrix for daily stock returns
returnsMat = matrix(NA, length(unique_dates), length(unique_tickers))

# Calculate daily returns (price on day j - price on day j-1)
for (i in 1:length(unique_tickers)){
  for (j in 2:length(unique_dates)){
    returnsMat[j, i] = priceMat[j, i] - priceMat[j-1, i]
  }
}

rownames(returnsMat) = as.character(unique_dates)

# Remove first row of returns matrix as it contains all NA values
returnsMat = returnsMat[-1,]
```
...
## use="complete" removes rows of the returns matrix with missing values
corMat = cor(returnsMat, use="complete")
```
